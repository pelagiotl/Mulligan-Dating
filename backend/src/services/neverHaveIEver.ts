...
export async function startNewGame(
  ...
  if (row) {
    const updateResult = db.prepare(
      `UPDATE never_have_i_ever_games SET user1_strikes = 0, user2_strikes = 0, current_prompt = ?, user1_answer = NULL, user2_answer = NULL, updated_at = ? WHERE match_id = ?`
    ).run([prompt, ts, matchId]);
    if (updateResult instanceof Promise) await updateResult;
  } else {
    const insertResult = db.prepare(
      `INSERT INTO never_have_i_ever_games (match_id, user1_strikes, user2_strikes, spice_level, current_prompt, user1_answer, user2_answer, updated_at) VALUES (?, 0, 0, ?, ?, NULL, NULL, ?)`
